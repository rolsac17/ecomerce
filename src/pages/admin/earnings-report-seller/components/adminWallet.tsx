...
const AdminWallet = ({ transactions, tableRef }: { transactions: any, tableRef:any }) => {
  ...
  const totalToPayment = ((acumulative:any, paid:any) => {
      return  (paid > acumulative) ? (parseFloat(paid) - parseFloat(acumulative)):(parseFloat(acumulative) - parseFloat(paid));
  })
              ...
              <>
                {(c.sends > 0 || c.returned > 0 || c.delivered > 0) && (
                  <tr key={index} className="bg-white">
                    ...
                    <td className="whitespace-nowrap py-4 pr-3 text-sm font-normal  text-gray-600">
                      Q {parseFloat(c.cumulativeTotal).toFixed(2)}
                    </td>
                    <td className="whitespace-nowrap py-4 pr-3 text-sm font-normal  text-gray-600">
                      Q {parseFloat(c.paid).toFixed(2)}
                    </td>
                    <td className="whitespace-nowrap py-4 pr-3 text-sm font-normal  text-gray-600">
                      Q {(totalToPayment(c.cumulativeTotal, c.paid)).toFixed(2)}
                    </td>
                    <td className="whitespace-nowrap py-4 pr-3 text-sm font-normal  text-gray-600">
                      {(c.cumulativeTotal - c.paid) == 0 ? (
