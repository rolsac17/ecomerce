...
const EarningsReportWH = () => {
  ...
  const getPaymentsMade = () => {
    if (user !== null) {
      fetch(endPoints.reports.getPaymentsMadeSeller, {
        headers: {
          Authorization: 'Bearer ' + user.token,
        },
      })
          ...
          <div className="py-8 sm:flex sm:items-center">
            <div className="sm:flex-auto">
              <h1 className="text-3xl font-semibold text-gray-900">
                Reporte de actividades por vendedor
              </h1>
              <p className="mt-2 text-sm text-gray-700">
                Monitorea los pedidos realizados, los pagos y lo que se debe al
                vendedor en tiempo real.
              </p>
            </div>
            <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
              <DownloadTableExcel
                filename="Reporte Vendedores"
                sheet="vendedores"
          ...
          <div className="mx-4 mt-5 rounded-2xl border-1 border-gray-50  border-rounded  h-auto shadow-sm  ring-gray-100 sm:-mx-6 md:mx-0 md:rounded-lg">
            <div>
              {userType === 'ADMINISTRATOR' && (
                <AdminWallet tableRef={tableRef} transactions={transactions} />
              )}
            </div>
          </div>
        </div>
